...
model = DeterministicViT(vit, num_classes=10).to(device)
...
for epoch in range(num_epochs):
    ...
    train_acc, train_nll, train_ece = evaluate(model, trainloader, "Train", device)
    test_acc, test_nll, test_ece = evaluate(model, testloader, "Test", device)
    print(f"Epoch {epoch+1}/{num_epochs}, Loss: {running_loss / len(trainloader):.4f}, "
          f"Train Acc: {train_acc:.2f}%, Test Acc: {test_acc:.2f}%")
    
    with open(f"logs/training_log_DetViT_lrhead{LEARNING_RATE_HEAD}.csv", mode="a", newline="") as file:
